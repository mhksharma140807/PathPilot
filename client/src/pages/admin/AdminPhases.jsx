...
function AdminPhases() {
  ...
  const [editingPhase, setEditingPhase] = useState(null); // null for create, object for edit
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError(null);
      const [phasesRes, careersRes] = await Promise.all([
        getAdminPhases(),
        getAdminCareers(),
      ]);
      ...
      if (phasesRes?.success) {
        ...
        setError(phasesRes?.message || "Failed to load phases");
      }
  ...
  // Filtered & Searched Phases
  const filteredPhases = useMemo(() => {
    return phases.filter((p) => {
      // Career filter
      if (
        selectedCareerFilter !== "all" &&
        p.career?._id !== selectedCareerFilter &&
        p.career !== selectedCareerFilter
      ) {
        return false;
      }
      ...
      const careerTitleMatch = p.career?.title?.toLowerCase().includes(query);

      return titleMatch || descMatch || careerTitleMatch;
    });
  }, [phases, selectedCareerFilter, statusFilter, searchTerm]);

  // Statistics
  const totalCount = phases.length;
  const activeCount = phases.filter((p) => p.isActive).length;
  const inactiveCount = totalCount - activeCount;
  const totalModulesCount = phases.reduce(
  ...
  const handleOpenEditModal = (phase) => {
    ...
    setFormData({
      career: phase.career?._id || phase.career || "",
      title: phase.title || "",
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!formData.career) {
      setFormError("Please select a career");
      return;
    }
    ...
    try {
      ...
      if (editingPhase) {
        const res = await updateAdminPhase(editingPhase._id, payload);
        if (res?.success) {
          toast.success(`Phase "${res.phase.title}" updated successfully!`);
          setIsFormOpen(false);
          fetchData();
        } else {
          setFormError(res?.message || "Failed to update phase");
        }
      } else {
        const res = await createAdminPhase(payload);
        if (res?.success) {
          toast.success(`Phase "${res.phase.title}" created successfully!`);
          setIsFormOpen(false);
          fetchData();
        } else {
          setFormError(res?.message || "Failed to create phase");
        }
      }
    } catch (err) {
      setFormError(
  ...
  const handleToggleStatus = async (phase) => {
    try {
      const res = await toggleAdminPhaseStatus(phase._id, !phase.isActive);
      if (res?.success) {
        toast.success(res.message);
        setPhases((prev) =>
          prev.map((p) =>
            p._id === phase._id ? { ...p, isActive: res.phase.isActive } : p
          )
        );
      } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await deleteAdminPhase(deleteTarget._id);
      if (res?.success) {
        toast.success(`Phase "${deleteTarget.title}" deleted.`);
      ...
      if (err.response?.status === 409) {
        setDeleteConflict(
          errorData?.message ||
            "Cannot delete phase due to existing curriculum dependencies."
        );
      } else {
    ...
    <div className="mx-auto max-w-7xl space-y-8 font-sans antialiased">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-slate-500">
            Organize career roadmap sequence, phase order, and curriculum structure.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Phases</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Active Phases</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Inactive Phases</p>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
        <div className="relative w-full md:max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by title, description, or career..."
            value={searchTerm}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 bg-slate-50/70 outline-none transition focus:border-blue-500 max-w-[200px] truncate"
          >
        ...
        <>
          {filteredPhases.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center space-y-4">
              ...
              <div>
                <h3 className="text-base font-extrabold text-[#0F172A]">No Phases Found</h3>
                <p className="mt-1 text-xs text-slate-500">
                  {searchTerm || selectedCareerFilter !== "all" || statusFilter !== "all"
                    ? "Try adjusting your search criteria or filter options."
                    : "Get started by creating your first phase for a career path."}
                </p>
              </div>
              {!searchTerm && selectedCareerFilter === "all" && statusFilter === "all" && (
                <button
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/70 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                      ...
                      <th className="py-3.5 px-4">Phase Title & Career</th>
                      <th className="py-3.5 px-4">Description</th>
                      <th className="py-3.5 px-4 text-center">Status</th>
                      <th className="py-3.5 px-4 text-center">Curriculum Counts</th>
                      <th className="py-3.5 px-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-xs">
                    {filteredPhases.map((phase) => (
                      <tr key={phase._id} className="hover:bg-slate-50/80 transition">
                        {/* Order Badge */}
                        <td className="py-4 px-4 text-center align-top">
                          <span className="inline-flex h-7 w-7 items-center justify-center rounded-xl bg-amber-50 text-amber-700 font-extrabold text-xs border border-amber-200">
                            #{phase.order}
                          </span>
                        </td>
                        ...
                        <td className="py-4 px-4 align-top max-w-[220px]">
                          <p className="font-extrabold text-[#0F172A] leading-snug">{phase.title}</p>
                          <span className="inline-block mt-1 font-semibold text-[10px] text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                            {phase.career?.title || "Unknown Career"}
                          </span>
                        </td>
                        ...
                        <td className="py-4 px-4 text-center align-top">
                          ...
                          >
                            <span className={`h-1.5 w-1.5 rounded-full ${phase.isActive ? "bg-emerald-500" : "bg-slate-400"}`} />
                        ...
                        <td className="py-4 px-4 text-center align-top">
                          <div className="flex justify-center gap-2">
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100">
                              ...
                              <span>{phase.moduleCount ?? 0}</span>
                            </span>
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-700 bg-amber-50 px-2 py-1 rounded-lg border border-amber-100">
                              <span className="text-amber-400">Reqs:</span>
                              <span>{phase.requirementCount ?? 0}</span>
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-4 text-right align-top">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              type="button"
                              onClick={() => handleOpenEditModal(phase)}
                ...
                >
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-amber-50 text-amber-700 font-extrabold text-xs border border-amber-200">
                          #{phase.order}
                        </span>
                        <div>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-3 text-slate-500 font-semibold">
                      <span>{phase.moduleCount ?? 0} Modules</span>
                      <span>•</span>
                      <span>{phase.requirementCount ?? 0} Reqs</span>
                    </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {editingPhase
                    ? `Update details for "${editingPhase.title}"`
                    : "Add a new phase to a career roadmap."}
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4 text-xs">
              <div>
                <label className="mb-1 block font-bold text-[#0F172A] uppercase tracking-wider text-[11px]">
                  Associated Career <span className="text-red-500">*</span>
                </label>
                <select
                  name="career"
                  required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div className="sm:col-span-3">
                  ...
                  <input
                    ...
                    onChange={handleFormChange}
                    placeholder="e.g. Fundamentals & Basics"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleFormChange}
                  placeholder="Summarize key learning focus for this phase..."
                  className="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-xs text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 font-medium"
                />
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <input
                  type="checkbox"
                  id="phaseActiveToggle"
                  name="isActive"
                  checked={formData.isActive}
                  onChange={handleFormChange}
                  className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
                <label htmlFor="phaseActiveToggle" className="font-bold text-[#0F172A] cursor-pointer">
                  Activate phase for career roadmap immediately
                </label>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                >
                  {formSubmitting ? (
                    ...
                    <span>{editingPhase ? "Update Phase" : "Create Phase"}</span>
                  )}
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="relative w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl space-y-5">
            <div className="flex items-center gap-3 text-red-600">
              ...
              <div>
                <h3 className="text-base font-extrabold text-[#0F172A]">Delete Phase</h3>
                <p className="text-xs text-slate-500">
            ...
            {deleteConflict ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs space-y-2 text-amber-800">
                ...
                <p className="text-[11px] text-amber-700 italic">
                  Tip: Instead of deleting, you can deactivate this phase to hide it from student roadmaps.
                </p>
              </div>
            ) : (
              <p className="text-xs text-slate-600 leading-relaxed">
                Are you sure you want to delete <span className="font-bold">{deleteTarget.title}</span>? This action cannot be undone if the phase has no dependent curriculum data.
              </p>
            )}
